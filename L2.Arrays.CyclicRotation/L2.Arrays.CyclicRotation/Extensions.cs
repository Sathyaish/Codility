...
namespace L2.Arrays.CyclicRotation
{
    public static class Extensions
    {
        ...
        public static T[] Copy<T>(this T[] array, int start, int till)
        {
            if (array == null) throw new ArgumentNullException("array");

            var len = array.Length;
            if (till >= len) throw new ArgumentOutOfRangeException("till");
            if (start < 0) throw new ArgumentOutOfRangeException("start");
            if (till < start) throw new ArgumentOutOfRangeException("The argument 'start' must be less than or equal to the argument 'till'.", (Exception)null);

            var newLen = till - start + 1;
            var newArray = new T[newLen];
            Array.Copy(array, start, newArray, 0, newLen);
            return newArray;
        }
    }
}
